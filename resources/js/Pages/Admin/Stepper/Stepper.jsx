  ...
  const displaySteps = newStep.map((step, index) => {
    ...
    return (
      ...
      >
        <div className=" relative flex flex-col  items-center text-[#6941C6]">
          <div className={`rounded-full  transition duration-500 ease-in-out  border-2  border-gray-300 h-8 sm:h-10 w-8 sm:w-10 
           items-center justify-center sm:p-1 ${step.selected
              ? "bg-[#6941C6] text-white font-bold border  border-[#6941C6]" : " bg-[#F4EBFF]"
            }`}
          >
            {/* Display number */}
        ...
        <div className={`flex-auto border-t-4  sm:border-t-8  transition  duration-500  ease-in-out ${step.completed ? "border-[#6941C6]" : "border-[#E9D7FE]"
          }`}>
          {/* Display line */}
        </div>
      </div>
